...
function getNextFriday(date) {
  ...
  const addDays = (weekLength - currentDay + friday) % weekLength || weekLength;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const timestampDayDifference = new Date(dateEnd) - new Date(dateStart);
  const dayDifference = timestampDayDifference / 1000 / 60 / 60 / 24;
...
function isDateInPeriod(date, period) {
  ...
  return new Date(end) >= new Date(date) && new Date(start) <= new Date(date);
}
...
function formatDate(date) {
  ...
  const month = unformattedDate.getUTCMonth() + 1;
  const day = unformattedDate.getUTCDate();
  const unformattedHour = unformattedDate.getUTCHours();
  const postfix = unformattedHour >= 12 ? 'PM' : 'AM';
  const hour = unformattedHour % 12 || 12;
  const minutes = String(unformattedDate.getUTCMinutes()).padStart(2, '0');
  const seconds = String(unformattedDate.getUTCSeconds()).padStart(2, '0');

  return `${month}/${day}/${year}, ${hour}:${minutes}:${seconds} ${postfix}`;
}
...
function getCountWeekendsInMonth(month, year) {
  const monthLenth = new Date(year, month, 0).getDate();

  let weekendCount = 0;
  for (let i = 1; i <= monthLenth; i += 1) {
    const date = new Date(year, month - 1, i).getDay();
    if (date === 0 || date === 6) weekendCount += 1;
  }
  return weekendCount;
}
...
function getWeekNumberByDate(date) {
  ...
  const dayOfYear = (date - yearStart) / (1000 * 60 * 60 * 24) + 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const [endDay, endMonth, endYead] = end.split('-');

  const startDate = new Date(startYear, startMonth - 1, startDay);
  const endDate = new Date(endYead, endMonth - 1, endDay);
  const schedule = [];
